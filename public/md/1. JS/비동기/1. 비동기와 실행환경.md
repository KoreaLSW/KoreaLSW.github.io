...
<b>비동기(Asynchronous)</b> 프로그래밍은 작업을 순차적으로 기다리지 않고, 다른 작업을 수행하면서 결과를 기다리는 방식입니다.

### 동기 vs 비동기

<b>동기(Synchronous) 방식:</b>
...
- <b>사용자 경험 향상</b>: 네트워크 요청 중에도 UI가 멈추지 않음
- <b>성능 최적화</b>: 여러 작업을 동시에 처리 가능
- <b>효율적인 리소스 활용</b>: 대기 시간 동안 다른 작업 수행
...
1. <b>Memory Heap (메모리 힙)</b>
2. <b>Call Stack (콜 스택)</b>
3. <b>Event Loop (이벤트 루프)</b>
4. <b>Task Queue (태스크 큐)</b>

### Memory Heap (메모리 힙)

<b>메모리 힙</b>은 객체, 배열, 함수 등이 저장되는 메모리 영역입니다.
...
<b>콜 스택</b>은 함수 호출이 쌓이는 스택 구조입니다. JavaScript는 단일 스레드이므로 하나의 콜 스택만 존재합니다.
...
<b>이벤트 루프</b>는 콜 스택과 태스크 큐를 연결하는 핵심 메커니즘입니다.
...
<b>태스크 큐</b>는 비동기 작업의 콜백 함수가 대기하는 큐입니다.

<b>종류:</b>

1. <b>Macrotask Queue (매크로태스크 큐)</b>

   - `setTimeout`, `setInterval` 콜백
   - 이벤트 핸들러
   - I/O 작업

2. <b>Microtask Queue (마이크로태스크 큐)</b>
   - `Promise.then()` 콜백
   - `queueMicrotask()` 콜백
   - `MutationObserver` 콜백
